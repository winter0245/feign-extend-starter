...
public class BasicClientTest {
    ...
    public void testJson() {
        val dto = new Dto("Artem", 11, new Dto());
    ...
    public void testMultipleFilesList() throws Exception {
        ...
        val stringResponse = API.uploadWithList(new FeignFileList<>(asList(path1.toFile(), path2.toFile())));
        Assert.assertEquals(Files.size(path1) + Files.size(path2), Long.parseLong(stringResponse));
    }

    @Test
    public void testUploadWithDto() throws Exception {
        Dto grandSub = new Dto("grandSub", 0, null);
        val dto = new Dto("Artem", 11, new Dto("sun", 2, grandSub));
    ...
    @Test
    public void testUploadWithDtos() throws Exception {
        Dto grandSub = new Dto("grandSub", 0, null);
        val dto = new Dto("Artem", 11, new Dto("sun", 2, grandSub));
        SpringDto springDto = new SpringDto();
        springDto.setField1("11");
        springDto.setField2(2);
        val path = Paths.get(Thread.currentThread().getContextClassLoader().getResource("file.txt").toURI());
        Assert.assertTrue(Files.exists(path));

        val response = API.uploadWithDtos(dto, springDto, path.toFile());
        Assert.assertNotNull(response);
        Assert.assertEquals(200, response.status());
    }
